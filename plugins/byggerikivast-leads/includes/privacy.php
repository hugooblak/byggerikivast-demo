...
add_filter(
	...
	function ( $exporters ) {
		$exporters['byggerikivast-leads'] = array(
			'exporter_friendly_name' => __( 'Quote requests', 'byggerikivast-leads' ),
			'callback'               => function ( $email ) {
				$items = array();
				foreach ( npl_leads_by_email( $email ) as $lead ) {
					$data = array( array( 'name' => __( 'Received', 'byggerikivast-leads' ), 'value' => $lead->post_date ) );
					...
					$items[] = array(
						'group_id'    => 'byggerikivast-leads',
						'group_label' => __( 'Quote requests', 'byggerikivast-leads' ),
...
add_filter(
	...
	function ( $erasers ) {
		$erasers['byggerikivast-leads'] = array(
			'eraser_friendly_name' => __( 'Quote requests', 'byggerikivast-leads' ),
			'callback'             => function ( $email ) {
				$removed = false;
				foreach ( npl_leads_by_email( $email ) as $lead ) {
