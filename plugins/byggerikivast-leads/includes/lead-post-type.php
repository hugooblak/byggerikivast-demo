...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'byggerikivast-leads' ),
					'singular_name' => __( 'Lead', 'byggerikivast-leads' ),
					'edit_item'     => __( 'Förfrågan', 'byggerikivast-leads' ),
					'search_items'  => __( 'Search leads', 'byggerikivast-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'byggerikivast-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'byggerikivast-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'byggerikivast-leads' ),
			'npl_phone'    => __( 'Phone', 'byggerikivast-leads' ),
			'npl_service'  => __( 'Needs', 'byggerikivast-leads' ),
			'npl_timeline' => __( 'When', 'byggerikivast-leads' ),
			'npl_zip'      => __( 'ZIP', 'byggerikivast-leads' ),
			'npl_source'   => __( 'Source', 'byggerikivast-leads' ),
			'npl_status'   => __( 'Status', 'byggerikivast-leads' ),
			'date'         => __( 'Received', 'byggerikivast-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'byggerikivast-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'byggerikivast-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'byggerikivast-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'byggerikivast-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'byggerikivast-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'byggerikivast-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'byggerikivast-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'byggerikivast-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'byggerikivast-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'byggerikivast-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'byggerikivast-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'byggerikivast-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
